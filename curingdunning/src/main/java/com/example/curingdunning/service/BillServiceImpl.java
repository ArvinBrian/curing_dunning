...
public class BillServiceImpl implements BillService {
    ...
    @Autowired
    private CuringActionRepository curingActionRepo;
	        ...
	        for (ServiceSubscription sub : subscriptions) {
	            ...
	            if (sub.getPlanType() == PlanType.POSTPAID) {
	                ...
	                if (!today.isBefore(nextPaymentDate)) {
	                    ...
	                    boolean hasPendingDunningEvent = eventRepo.existsByCustomer_CustomerIdAndSubscription_ServiceNameAndStatus(
	    ...
	    for (DunningEvent ev : pendingEvents) {
	        ...
	        ServiceSubscription sub = subRepo
	            .findByCustomerCustomerIdAndServiceNameAndPlanTypeAndNextPaymentDate(
	                c.getCustomerId(),
	                ev.getSubscription().getServiceName(), // Get serviceName from the linked subscription
	        ...
	        Bill b = new Bill();
	        b.setCustomer(c);
	        b.setServiceName(ev.getSubscription().getServiceName()); // Get serviceName from the linked subscription
	        b.setAmount(amount);
	        ...
	        log.info("🚨 Overdue bill generated for customer={} service={}", c.getCustomerId(), ev.getSubscription().getServiceName());
	    }
...
//        dto.setPlanType(subRepo.getPlanType().name());
	...
	@Override
	@Transactional // Ensure atomicity for all database operations
	public Bill markBillAsPaid(Bill bill) {
	    ...
	    // --- 2. Resolve ONLY the Linked Dunning Event ---
	    DunningEvent linkedEvent = bill.getDunningEvent();
	    
	    if (linkedEvent != null && "PENDING".equalsIgnoreCase(linkedEvent.getStatus())) {
	        linkedEvent.setStatus("RESOLVED");
	        linkedEvent.setResolvedAt(LocalDateTime.now());
	        eventRepo.save(linkedEvent);

	        log.info("Dunning Event ID {} resolved upon payment of Bill ID {}.", 
	            linkedEvent.getId(), bill.getBillId());
	            
	        // --- 3. Reactivate Subscription and Update Cycle ---
	        // Find the specific subscription linked by the event's customer and service.
	        List<ServiceSubscription> subs = subRepo.findByCustomerCustomerIdAndServiceName(
	            customer.getCustomerId(),
	            linkedEvent.getSubscription().getServiceName() // Use the service name from the resolved event's subscription
	        );

	        if (!subs.isEmpty()) {
	            ServiceSubscription sub = subs.get(0); // Take the first matching sub
	            
	            // Reactivate service only if it was blocked by a Dunning Rule
	            if ("BLOCKED".equals(sub.getStatus())) {
	                sub.setStatus("ACTIVE");
	            }
	            // Update cycle dates for this specific subscription
	            sub.setLastPaymentDate(LocalDate.now());
	            sub.updateNextPaymentDate();
	            subRepo.save(sub);
	            log.info("Subscription ID {} restored to ACTIVE and cycle updated.", sub.getId());
	        }
	    } 
	    
	    // --- 4. Record Curing Action ---
	    CuringAction action = new CuringAction();
	    action.setCustomer(customer);
	    action.setDunningEvent(linkedEvent); // Can be null
	    action.setPaidBill(bill);
	    action.setActionType(ActionType.MAKE_PAYMENT); // Use the Enum
	    action.setStatus(com.example.curingdunning.entity.ActionStatus.COMPLETED); // Set status to COMPLETED
	    action.setCreatedAt(LocalDateTime.now()); // Set the creation timestamp
	    action.setCuredAt(LocalDateTime.now());
	    curingActionRepo.save(action);
	    log.info("Curing action '{}' recorded as COMPLETED for Bill ID: {}", action.getActionType(), bill.getBillId());
